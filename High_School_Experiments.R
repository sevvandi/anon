...
# contacts-prox-high-school-2013-attr dataset
...
dat <- read.csv('Data_Input/contacts-prox-high-school-2013-attr.edges', header = FALSE)
head(dat)
dat2 <- dat[ ,1:3]
colnames(dat2) <- c("From", "To", "Seconds")
length(unique(dat2$Seconds))

# Change seconds to hour - call it time
dat2 <- dat2 %>%
  mutate(Time = ceiling((Seconds - min(Seconds) + 1)/(60*60)) )
head(dat2)
...
filename <- paste("Data_Output/High_School/Evaluation_Metrics_High_School_array_index_", array_index,  ".csv", sep="" )
